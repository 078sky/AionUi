...
interface SiderProjectsEntryProps {
  isMobile: boolean;
  isActive: boolean;
  collapsed: boolean;
...
const SiderProjectsEntry: React.FC<SiderProjectsEntryProps> = ({
  ...
  if (collapsed) {
    return (
      <Tooltip {...siderTooltipProps} content={t('project.pageTitle', { defaultValue: 'Projects' })} position='right'>
        <div
    ...
    <Tooltip {...siderTooltipProps} content={t('project.pageTitle', { defaultValue: 'Projects' })} position='right'>
      ...
      >
        ...
        <span className='collapsed-hidden text-t-primary text-14px font-medium leading-22px'>
          {t('project.pageTitle', { defaultValue: 'Projects' })}
        </span>
      </div>
    </Tooltip>
  );
