 ...
 * Update Handler
 *
 * Manages GitHub release checking, asset downloading, and electron-updater
 * auto-update flow. Replaces initUpdateBridge() from
...
const startDownloadInBackground = async (
  ...
  filePath: string,
  abortController: AbortController,
  emitProgress: (evt: UpdateDownloadProgressEvent) => void
) => {
  let receivedBytes = 0;
...
/**
 * Create a status broadcast callback that sends auto-update status via router.emit.
 */
export function createAutoUpdateStatusBroadcast(router: WsRouter): (status: AutoUpdateStatus) => void {
  return (status) => {
    router.emit('auto-update.status', status);
  };
}

/**
 * Register update endpoint handlers on the WsRouter.
 * Replaces initUpdateBridge() from src/process/bridge/updateBridge.ts.
 */
export function registerUpdateHandlers(router: WsRouter): void {
  router.handle('update.check', async (params) => {
    try {
      const repo = resolveRepo(params?.repo);
      const includePrerelease = Boolean(params?.includePrerelease);
      const currentVersion = getPlatformServices().paths.getVersion();
  ...
  router.handle('update.download', (params: UpdateDownloadRequest) => {
    try {
      if (!params?.url) {
      ...
      const downloadsDir = getPlatformServices().paths.getSystemPath('downloads');
      if (!downloadsDir) {
        return Promise.resolve({ success: false, msg: 'Cannot determine downloads directory' });
      }
      ...
      const emitProgress = (evt: UpdateDownloadProgressEvent) => {
        router.emit('update.download.progress', evt);
      };

      // Start background download, but return immediately so the UI stays responsive.
      void startDownloadInBackground(downloadId, params.url, targetPath, abortController, emitProgress);

      return Promise.resolve({
        success: true,
        data: { downloadId, filePath: targetPath } satisfies UpdateDownloadResult,
      });
    } catch (err: unknown) {
  ...
  router.handle('auto-update.check', async (params) => {
    try {
      const includePrerelease = Boolean(params?.includePrerelease);
  ...
  router.handle('auto-update.download', async () => {
    try {
  ...
  router.handle('auto-update.quit-and-install', async () => {
    try {
