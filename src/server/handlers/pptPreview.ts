...
function checkForUpdate(): void {
  ...
  try {
    ...
    // File doesn't exist yet — proceed with check
  }
 ...
 * Emits 'ppt-preview.status' installing event via the router.
 */
function installOfficecli(router?: WsRouter): boolean {
  try {
    router?.emit('ppt-preview.status', { state: 'installing' });
 ...
 */
async function startWatch(filePath: string, router: WsRouter, retry = false): Promise<string> {
  ...
  router.emit('ppt-preview.status', { state: 'starting' });
  ...
  return new Promise<string>((resolve, reject) => {
    ...
    child.on('error', (err) => {
      ...
      if ((err as NodeJS.ErrnoException).code === 'ENOENT' && !retry) {
        ...
        clearTimeout(timeout);
        if (installOfficecli(router)) {
          settled = true;
          startWatch(filePath, router, true).then(resolve, reject);
        } else {
...
/**
 * Register PPT preview endpoint handlers on the WsRouter.
 * Replaces initPptPreviewBridge() from src/process/bridge/pptPreviewBridge.ts.
 */
export function registerPptPreviewHandlers(router: WsRouter): void {
  // Background update check (non-blocking, at most once per day)
  setTimeout(() => checkForUpdate(), 5000);

  router.handle('ppt-preview.start', async ({ filePath }) => {
    const result = await startWatch(filePath, router)
      .then((url) => ({ url }))
      .catch((err: unknown) => {
  ...
  router.handle('ppt-preview.stop', async ({ filePath }) => {
    try {
      filePath = fs.realpathSync(filePath);
    } catch {
