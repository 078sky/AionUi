...
/**
 * Register remote-agent endpoint handlers on the WsRouter.
 * Replaces initRemoteAgentBridge() from src/process/bridge/remoteAgentBridge.ts.
 */
export function registerRemoteAgentHandlers(router: WsRouter): void {
  router.handle('remote-agent.list', async () => {
    const db = await getDatabase();
    return db.getRemoteAgents();
  });

  router.handle('remote-agent.get', async ({ id }) => {
    const db = await getDatabase();
    return db.getRemoteAgent(id);
  });

  router.handle('remote-agent.create', async (input) => {
    const db = await getDatabase();
    const now = Date.now();
  ...
  router.handle('remote-agent.update', async ({ id, updates }) => {
    const db = await getDatabase();
  ...
  router.handle('remote-agent.delete', async ({ id }) => {
    const db = await getDatabase();
    const result = db.deleteRemoteAgent(id);
    return result.success;
  });

  router.handle('remote-agent.test-connection', async ({ url, authType, authToken, allowInsecure }) => {
  ...
  router.handle('remote-agent.handshake', async ({ id }) => {
