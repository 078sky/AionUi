...
/**
 * Register Bedrock endpoint handlers on the WsRouter.
 * Replaces initBedrockBridge() from src/process/bridge/bedrockBridge.ts.
 */
export function registerBedrockHandlers(router: WsRouter): void {
  // Test AWS Bedrock connection with provided credentials
  router.handle('bedrock.test-connection', async ({ bedrockConfig }) => {
    try {
